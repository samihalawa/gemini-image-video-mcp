...
export type ToolExecutionCallback = (newOutput: string) => void;

export interface UnifiedTool {
  name: string;
  description: string;
  zodSchema: ZodTypeAny;
  category: "image-generation" | "video-generation" | "media-management" | "text-processing";
  ...
  execute: (
    ...
    onProgress?: ToolExecutionCallback
  ) => Promise<string>;
}
...
export async function executeTool(
  ...
  try {
    ...
    if (onProgress) {
      onProgress(`Starting ${toolName}...`);
    }
    
    const result = await tool.execute(validatedArgs, geminiClient, onProgress);
